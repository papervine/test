...
- The **control plane** shares one dark, luminous design language codified in
  ...
  same choice the incumbent made for their own dashboard. The neutral tokens resolve only inside
...
<Info>
  ...
  doubles as the self-host / no-custom-domain story. When a real domain is added, subdomain
...
<Steps>
  ...
  <Step title="docs.acme.com — the customer's domain, a cert per host">
    ...
    a `CNAME docs.acme.com → {branded target}` (apex domains use an `A` record, since you
...
<Tip>
  **Why a branded CNAME target, not the host's edge directly.** Customers CNAME at a record
  ...
  open-source code hardcodes no operator domain — a self-hoster sets their own.
</Tip>
