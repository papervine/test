...
<Steps>
  <Step title="Compile with @mintlify/mdx's serialize">
    `serialize` is the third-party MDX serializer — it brings Shiki dual-theme syntax highlighting
    and snippet handling, so highlighting matches the incumbent exactly. It produces compiled,
    serializable output rather than a rendered element.
  </Step>
  <Step title="Execute with @mdx-js/mdx's run(), inside a try/catch">
...
The obvious simplification — the incumbent's `MDXRemote` — is wrong here. `MDXRemote` throws
...
<Info>
  ...
  renders an **inline notice** instead of a 500. Layered together, these mean any real
...
- **`legacy-peer-deps`** — `@mintlify/mdx@4` declares a broken peer dependency
