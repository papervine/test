...
`<Accordion>` is a single collapsible disclosure; `<AccordionGroup>` draws several as one
bordered list, divided by hairlines, so a related set reads as a set. Use them to hide secondary
detail (FAQs, advanced options) until the reader expands it.
