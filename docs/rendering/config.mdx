...
description: How Papervine parses docs.json as a docs.json-compatible compatibility layer and walks its recursive navigation tree.
---

A single `docs.json` at the repo root configures a Papervine site. It mirrors the incumbent's
schema so an existing docs.json repo migrates unchanged. The core shape:
...
This matters because real docs.json configs use keys and shapes Papervine has not implemented
...
<Warning>
  ...
  Lenient parsing is a feature: it is what lets unmodified docs repos render. Add support
...
`navigation` is **one recursive structure**, matching the incumbent's docs.json. It is not a flat
...
`linden`, `almond`, `aspen`, `sequoia`, or `luma` (the incumbent's set). Each preset is a small
...
- **`appearance`** controls light/dark: `{ "default": "light" | "dark" | "system", "strict":
  ...
  the OS), and `strict` hides the light/dark switcher. The default is light, matching the incumbent.
