...
```jsonc
{
  ...
  "navbar": {
    "links": [{ "label": "Community", "href": "https://..." }],
    "primary": { "label": "Dashboard", "href": "..." }
  },
...
<Accordion title="Navigation layers, top to bottom">
  - **`languages`** — top-level locale split (e.g. `en`, `es`); each language carries its own
    subtree. **Only the first entry renders** — there is no switcher yet.
  - **`versions`** — product/version split within a language. Same rule: first entry only.
  - **`tabs`** — render as a horizontal tab bar (from two tabs up); the sidebar is scoped to
    the active tab.
  - **`anchors` / `dropdowns`** — labeled containers of pages, rendered as sidebar sections.
    They are not link slots: an anchor with a `href` and no pages is dropped. Off-site links
    are `navbar.links`, and `navigation.global` is parsed but not rendered.
  - **`groups`** — collapsible sidebar sections; support `group.root`, `group.icon`,
    `group.tag`, `group.expanded` and `group.hidden`.
...
`seo.metatags` is an open name → content map emitted in the `<head>` of every page:
...
Tags Papervine models itself (`og:image`, `twitter:card`, `og:title`, …) are folded into the
metadata it already builds; anything else is passed through verbatim. A page's own frontmatter
overrides the site-wide value. See [Social cards & SEO](/guides/social-cards) for what a page
emits by default and how the generated card is built.
