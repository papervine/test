...
<Card title="Deploy with Vercel" icon="triangle" href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fpapervine%2Fpapervine&root-directory=apps%2Fcli">
  Forks `papervine/papervine` and deploys `apps/cli`, serving the starter site. Replace
...
<Note>
  ...
  (`papervine build`) is on the [roadmap](/roadmap) and is the better fit at scale.
</Note>

The rest of this page is for running it yourself, which gives you caching, a single long-lived
process, and no per-request billing.

## Dokploy, Railway, Render and friends

Platforms that build from a repo rather than a Dockerfile run `npm install` and then
`npm start`. A site created by [`papervine new`](/cli) is ready for that with no configuration:
its `package.json` already depends on `papervine` and starts it.
...
Point the platform at your docs repo and deploy. Nothing else to set.

<Note>
  `serve` binds the port the platform assigns through `$PORT`, and every interface, which is what
  a health check needs to reach it. If you pass `--port` it wins over `$PORT` — so don't, on a
  platform that assigns one.
</Note>

Adding Papervine to a docs repo you already have is the same two lines:

```bash
npm init -y          # if it has no package.json yet
...
The [assistant](/features/assistant-providers) and the
[read MCP server](/features/mcp-servers) are **not** on that list — a self-hosted site serves
...
<Warning>
  **A repo carrying `groups:` frontmatter is fully public when self-hosted.** Reader auth is the
  ...
  The [MCP server](/features/mcp-servers) sharpens this: `list_pages` returns **every** page in one
  ...
  If your repo has gated pages, either keep them out of the deployment or use
  [hosted Papervine](/guides/publishing), where the gate is real.
</Warning>
