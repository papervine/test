---
title: CLI
description: The papervine CLI is a local docs previewer — the mint dev analogue — that renders any MDX + docs.json repo on your machine.
---

`papervine` is a **local development tool**, the analogue of the incumbent's `mint dev`. You run
it inside a folder of MDX + a `docs.json` and it boots the Papervine renderer pointed at that
folder, with live reload — the same renderer that powers hosted Papervine sites, so what you
see locally is what you ship.
...
<Note>
  The CLI is a previewer, not a deployer. Like `mint`, it has **no `deploy` and no `login`** —
  publishing happens through Git (push your repo) and the hosted control plane, not the CLI.
</Note>
...
exactly like `mint dev`. That's expected for your own docs, but it means you should only run
the CLI on docs repos you trust, the same way you'd only run `npm install` in a project you
trust.
