...
`dev` is the whole command surface today. Planned next: `broken-links` (dead internal links),
`validate` (a strict config/frontmatter gate for CI), `openapi-check`, `new` (scaffold from
the starter template), and `build` (static export).
