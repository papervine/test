...
compile step, no hot reload. That makes it equally suitable for previewing as you write and
for serving the finished site, which is why the command is the same either way. See
...
The server binds loopback (`127.0.0.1`) rather than every network interface, so it isn't
reachable from the rest of the network by default. Set `PAPERVINE_HOST=0.0.0.0` when you need to
reach it from outside the machine — from a container host, or when
[serving in production](#serving-a-site-in-production).
...
**Bind an interface.** The default is loopback, which is right for a command on your laptop
and wrong for a server. Set the host explicitly:
...
authentication, no analytics. Those are the control plane, and none of it is in this package —
