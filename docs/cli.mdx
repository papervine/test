...
`papervine dev` **compiles and runs the repo's MDX**, and an MDX expression is real JavaScript
executed on your machine while the page renders. Concretely, a page in the repo you point at can
read your environment variables, reach the network, and run commands as your user — the same
power a `postinstall` script has.

<Warning>
  **Only preview docs repos you trust**, with the same care you'd apply before running
  `npm install` in someone else's project. This isn't specific to Papervine — every docs
  generator that executes MDX or JSX at build time works this way — but it is worth knowing
  before you preview a pull request from a stranger.
</Warning>

Nothing about the previewer widens that: it binds loopback, serves only asset file types out of
your content directory, and refuses remote image URLs. The MDX execution *is* the trust
boundary, so the repo is the thing to be careful about.
