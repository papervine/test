...
enable; the SDKs ship with the CLI, and you bring a model.
...
<Note>
  To run it **free and entirely offline**, point it at any OpenAI-compatible server — Ollama,
  LM Studio, llama.cpp:
...
| `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, … | Your key, for `direct` routing |
