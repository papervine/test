...
Papervine is an open-source, multi-tenant **docs.json-compatible docs platform**: it renders a docs site from a
Git repo of MDX + a `docs.json`. This page is the contract for how to work in the repo —
...
<Steps>
  ...
  <Step title="Renderer / config / nav changes: crawl a representative docs repo">
    `node tests/crawl.mjs <cloned-incumbent-repo>` — expect 0 × HTTP 500.
  </Step>
...
<Steps>
  ...
  <Step title="Seed a known account">
    ...
    **`dev-password-123`**, an org, and a connected site `starter` (→ `papervine/starter`)
...
<Info>
  ...
  docs.json-compatible, so real repos migrate unchanged. When unsure how something should
  behave, check against a representative docs repo rather than guess.
</Info>
