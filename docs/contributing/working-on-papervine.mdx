...
npm run dev:full            # the whole stack in one command: docker services + the app + the
                            # trigger.dev worker (automations and cron ticks execute only while
                            # it runs). Each piece degrades independently if unavailable.
