...
<Note>
  ...
  live in these product docs, not on the storefront. Drive everything with
  **`agent-browser`** (the repo standard — `open` / `snapshot -i` / `click @eN` / `fill` /
  `screenshot`; it navigates `app.localhost` and `{slug}.localhost` fine) or Playwright.
  This is the same loop that catches per-request content-source bugs: connect a repo, open
  its docs, confirm the sidebar and pages are the tenant's, not the platform's.
</Note>
