---
title: Reader auth
description: A docs.json-compatible handshake that gates a customer's published docs for their own end users, without Papervine ever storing reader credentials.
---
...
The model matches the incumbent's exactly, so an existing incumbent "Authentication &
Personalization" configuration migrates unchanged. For how this differs from the dashboard
...
The signed payload is docs.json-compatible:
