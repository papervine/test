...
function pushDelivery(
  overrides: { ref?: string; sha?: string; message?: string; owner?: string; repo?: string } = {},
) {
...
test.beforeAll(async () => {
  ...
  await sql`insert into site (id, organization_id, name, slug, repo_owner, repo_name, branch, docs_path, status, last_synced_commit_sha)
            values (${SITE_ID}, ${org.id}, 'Webhook Site', 'e2e-webhook', ${REPO_OWNER}, ${REPO_NAME}, 'main', '', 'live', ${SYNCED_SHA})`;
  await sql.end();
});
