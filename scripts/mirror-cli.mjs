...
try {
  ...
  if (!INITIAL) {
    ...
    step(`checking the public repo still matches ${lastSha.slice(0, 12)}`);
    const expected = path.join(WORK, "expected");
    buildSnapshot(lastSha, expected);
    // `package-lock.json` is excluded on purpose: it's generated by `npm install
    // --package-lock-only`, whose output can shift with the npm version and registry state,
    // so comparing it would raise false divergence on a rebuild that changed nothing. This
    // guard is about whether someone edited *source* on the public repo.
    const diff = shSoft("diff", [
      "-r",
      "-q",
      "--exclude=.git",
    ...
    if (diff.code !== 0) {
      fail(
        `the public repo has diverged — it is NOT what this script last published:\n\n` +
          `${diff.out}\n\n` +
          `  Almost certainly a PR was merged on the public repo. Nothing here has been\n` +
          `  changed. Port those commits into the monorepo first (preserving authorship:\n` +
          `  \`git cherry-pick\` or \`git am\`, keeping --author), then re-run. Publishing\n` +
          `  over a divergence would silently revert that work, which is exactly what this\n` +
          `  check exists to prevent. Override only if you are certain: --initial.`,
      );
    }
    ok("no divergence");
  }
  ...
  for (const sha of commits) {
    ...
    const message = INITIAL
      ? `Initial import\n\nPublished from the Papervine monorepo at ${sha}.\n` +
