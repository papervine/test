...
function validateSnapshot(dir) {
  step("validating the snapshot (install + typecheck + unit)");
  const install = shSoft("npm", ["install", "--no-audit", "--no-fund"], { cwd: dir });
  if (install.code !== 0) {
    fail(`the generated snapshot does not install:\n${install.err || install.out}`);
  }
  ok("npm install");
