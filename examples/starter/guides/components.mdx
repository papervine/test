...
<Note>
  ...
  line. Use a Lucide name, or point `src` at any file of your own.
</Note>

`src` takes a path or a URL: <Icon src="/favicon.svg" size={20} />

<Warning>
  Pick an asset that reads on both light and dark backgrounds. A single-colour logo that works
  on one will disappear on the other — `favicon.svg` is used above because it has its own
  background.
</Warning>
...
Also available as `<FileTree>`. A Markdown list works too, and is usually less typing — a
trailing slash marks a folder, and so does having nested items:
